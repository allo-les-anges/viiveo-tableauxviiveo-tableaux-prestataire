...
window.openModalStartPrestation = function(missionId, clientPrenom, clientNom) { // Rendre la fonction globale explicitement
    console.log(`openModalStartPrestation appelée pour mission ID: ${missionId}`); // NOUVEAU LOG
...
function renderTable(missions, type = "") {
    ...
    missions.forEach(m => {
        ...
        if (type === "attente") {
            ...
            html += `<td data-label="Actions" class="actions"><button class="btn-action btn-start" onclick="console.log('Bouton action cliqué pour mission ${m.id}'); window.openModalStartPrestation('${m.id}', '${m.clientPrenom}', '${m.clientNom}')">▶️</button></td>`; // NOUVEAU LOG ET window.openModalStartPrestation
        }
        html += "</tr>";
    });
...
// Rendre la fonction callApiJsonp globale pour qu'elle soit accessible par les callbacks JSONP
window.callApiJsonp = function(url, callbackName) { // NOUVEAU: Rendre callApiJsonp globale
